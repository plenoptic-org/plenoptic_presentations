...
def synth_texture(max_iter=100, store_progress=True,
    ...
    met = po.Metamer(img, ps, loss_function=loss)
    opt_kwargs = {
        "max_iter": 10,
        "max_eval": 10,
        "history_size": 100,
...
def init_figure(image, model, ylim=None):
    ...
    po.plot.imshow(po.to_numpy(image), ax=axes[0], vrange=(0, 1), title=None)
    for ax in axes:
        ax.set_axis_off()
    po.plot.stem_plot(po.to_numpy(model(image).squeeze()), axes[2], ylim=False)
    if ylim is None:
...
def animate(met, framerate=10, save_path=None):
    ...
    def update_frame(i):
        artists = []
        artists.extend(
            po.plot.update_plot(fig.axes[0], data=saved_metamer[i],
                                batch_idx=0)
        )
        artists.extend(
            po.plot.update_plot(fig.axes[2],
